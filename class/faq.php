...
/**
* Classes responsible for managing FAQ faq objects
*
 ...
 */
define ( 'FAQ_FAQ_STATUS_PUBLISHED', 1 );
define ( 'FAQ_FAQ_STATUS_PENDING', 2 );
define ( 'FAQ_FAQ_STATUS_REJECTED', 3 );
define ( 'FAQ_FAQ_STATUS_OFFLINE', 4 );

class FaqFaq extends IcmsPersistableSeoObject {
	...
	public function __construct(& $handler) {
		...
		$this->setControl ( 'faq_status', array ('itemHandler' => 'faq', 'method' => 'getFaq_statusArray', 'module' => 'faq' ) );
		$this->setControl ( 'faq_cid', array ('itemHandler' => 'faq', 'method' => 'getFaq_categoriesArray', 'module' => 'faq' ) );
	...
	function faq_cid() {
		...
		$category = $faq_category_handler->get ( $cid );
	...
	function faq_attachments() {
		$faq_attachment_handler = icms_getModuleHandler ( 'attachment', 'faq' );
		
		$attachments = $faq_attachment_handler->getAttachments ( false, false, false, $this->getVar ( 'faq_id' ) );
	 ...
	 */
	function getFaqInfo() {
		$ret = sprintf ( _CO_FAQ_FAQ_INFO, $this->getPoster ( true ), $this->getVar ( 'faq_published_date' ), $this->getVar ( 'counter' ) );
		return $ret;
	}
	...
	function getItemLink($onlyUrl = true, $forprint = false, $title = null) {
		...
		$seoMode = faq_getModuleModeSEO ( $this->handler->_moduleName );
		$seoModuleName = faq_getModuleNameForSEO ( $this->handler->_moduleName );
		$seoIncludeId = faq_getModuleIncludeIdSEO ( $this->handler->_moduleName );
		
		$faq_category_handler = icms_getModuleHandler ( 'category', 'faq' );
		
		$categoryObj = $faq_category_handler->get ( $this->getVar ( 'faq_cid', 'e' ) );
		...
		if ($forprint) {
			if ($seoMode == 'rewrite') {
				...
				$ret = FAQ_URL . 'seo.php/' . $seoModuleName . '.' . $this->handler->_itemname . ($seoIncludeId ? '.' . $this->getVar ( $this->handler->keyName ) : '') . '/' . $this->getVar ( 'short_url' ) . '.html';
			} else {
				$ret = $this->handler->_moduleUrl . 'print.php' . "?" . $this->handler->keyName . "=" . $this->getVar ( $this->handler->keyName );
			}
			$ret = "javascript:openWithSelfMain('" . $ret . "','faq_print','700','519');";
		} else {
			if ($seoMode == 'rewrite') {
	...
	function getCommentsInfo() {
		...
		if ($faq_comments) {
			return '<a href="' . $this->getItemLink ( true ) . '#comments_container">' . sprintf ( _CO_FAQ_FAQ_COMMENTS_INFO, $faq_comments ) . '</a>';
		} else {
			return _CO_FAQ_FAQ_NO_COMMENT;
		}
	}
	...
	function accessGranted() {
		...
		if ($viewperm && $this->getVar ( 'faq_status', 'e' ) == FAQ_FAQ_STATUS_PUBLISHED) {
			return true;
		}
...
class FaqFaqHandler extends IcmsPersistableObjectHandler {
	...
	public function __construct(& $db) {
		$this->IcmsPersistableObjectHandler ( $db, 'faq', 'faq_id', 'faq_question', 'faq_answer', 'faq' );
	}
	...
	function getFaq_statusArray() {
		if (! $this->_faq_statusArray) {
			$this->_faq_statusArray [FAQ_FAQ_STATUS_PUBLISHED] = _CO_FAQ_FAQ_STATUS_PUBLISHED;
			$this->_faq_statusArray [FAQ_FAQ_STATUS_PENDING] = _CO_FAQ_FAQ_STATUS_PENDING;
			$this->_faq_statusArray [FAQ_FAQ_STATUS_REJECTED] = _CO_FAQ_FAQ_STATUS_REJECTED;
			$this->_faq_statusArray [FAQ_FAQ_STATUS_OFFLINE] = _CO_FAQ_FAQ_STATUS_OFFLINE;
		}
	...
	function getFaq_categoriesArray($permission = 'category_write', $shownull = false) {
		...
		$categories = $faq_category_handler->getCategoryList ( $groups, $permission, null, null, $shownull );
	...
	function getFaqsCriteria($start = 0, $limit = 0, $faq_cid = false, $faq_id = false, $faq_uid = false, $order = 'faq_published_date', $sort = 'DESC') {
		...
		$criteria->add ( new Criteria ( 'faq_status', FAQ_FAQ_STATUS_PUBLISHED ) );
	...
	function updateCounter($id) {
		...
		if (! is_object ( $xoopsUser ) || (! $faq_isAdmin && $faqObj->getVar ( 'faq_uid', 'e' ) != $xoopsUser->uid ())) {
	 ...
	 *
	 * This method is triggered by faq_com_update in include/functions.php which is
	...
	function afterSave(& $obj) {
		...
		if ($obj->getVar ( 'faq_status', 'e' ) == FAQ_FAQ_STATUS_PENDING && $obj->getVar ( 'faq_notifypub', 'e' )) {
		...
		if (! $obj->getVar ( 'faq_notification_sent', 'e' ) && $obj->getVar ( 'faq_status', 'e' ) == FAQ_FAQ_STATUS_PUBLISHED) {
		...
		$attachments = $faq_attachment_handler->getObjects ( $criteria, true, true );
		
		if (count ( $attachments ) > 0) {
			foreach ( $attachments as $attachment ) {
				$attachment->setVar ( 'attach_fid', $obj->getVar ( 'faq_id' ) );
				$faq_attachment_handler->insert ( $attachment, true );
			}
		}
	...
	function beforeDelete(& $obj) {
		...
		if (count ( $attachments ) > 0) {
			foreach ( $attachments as $attachment ) {
				if (! $faq_attachment_handler->delete ( $attachment, true )) {
					return false;
				}
			}
		}
