...
function editcategory($cat_id = 0, $cat_pid = 0) {
	global $faq_category_handler, $icmsModule, $icmsAdminTpl;
	
	$categoryObj = $faq_category_handler->get ( $cat_id );
	
	if (! $categoryObj->isNew ()) {
		$icmsModule->displayAdminMenu ( 2, _AM_FAQ_CATEGORYS . " > " . _CO_ICMS_EDITING );
		$sform = $categoryObj->getForm ( _AM_FAQ_CATEGORY_EDIT, 'addcategory' );
		$sform->assign ( $icmsAdminTpl );
	
	} else {
		$icmsModule->displayAdminMenu ( 2, _AM_FAQ_CATEGORYS . " > " . _CO_ICMS_CREATINGNEW );
		
		if ($cat_pid){
			$categoryObj->setVar('cat_pid',$cat_pid);
		}
		
		$sform = $categoryObj->getForm ( _AM_FAQ_CATEGORY_CREATE, 'addcategory' );
		$sform->assign ( $icmsAdminTpl );
	
	}
	$icmsAdminTpl->display ( 'db:faq_admin_category.html' );
}
...
if (in_array ( $clean_op, $valid_op, true )) {
	switch ( $clean_op) {
		...
		case "addcategory" :
			...
			$controller = new IcmsPersistableController ( $faq_category_handler );
			$controller->storeFromDefaultForm ( _AM_FAQ_CATEGORY_CREATED, _AM_FAQ_CATEGORY_MODIFIED );
		...
		case "changedField" :
		    foreach ($_POST['FaqCategory_objects'] as $k=>$v){
		       $changed = false;
		       $obj = $faq_category_handler->get($v);
		       ...
		       if ($changed){
		           $faq_category_handler->insert($obj);
		       }
		    }
		    if ($clean_cat_pid){
		    	$redir = '?cat_pid='.$clean_cat_pid;
		    }
		    redirect_header('category.php'.$redir,2,_AM_FAQ_CATEGORY_MODIFIED);
		...
		default :
			...
			$icmsModule->displayAdminMenu ( 2, _AM_FAQ_CATEGORYS );
			...
			$objectTable = new IcmsPersistableTable ( $faq_category_handler, $criteria );
			...
			$objectTable->addIntroButton ( 'addcategory', 'category.php?op=mod'.($clean_cat_pid?'&amp;cat_pid='.$clean_cat_pid:''), _AM_FAQ_CATEGORY_CREATE );
			...
			$icmsAdminTpl->assign ( 'faq_category_breadcrumb', $faq_category_handler->getBreadcrumbForPid ( $clean_cat_pid ) );			
			$icmsAdminTpl->assign ( 'faq_category_table', $objectTable->fetch () );
			$icmsAdminTpl->display ( 'db:faq_admin_category.html' );
		break;
	}
	icms_cp_footer ();
