...
if (in_array ( $clean_op, $valid_op, true )) {
	switch ($clean_op) {
		case "addattachments" :
			...
			$count = $faq_attachment_handler->getCount ( $criteria );
			
			$attachmentObj = $faq_attachment_handler->create ();
			...
			$faq_attachment_handler->insert ( $attachmentObj, true );
		...
		case "sort_attachments" :
			...
			foreach ( $order as $k => $v ) {
				$attachmentObj = $faq_attachment_handler->get ( $v );
		...
		case "show_hide_attachments" :
			...
			$attachmentObj = $faq_attachment_handler->get ( $clean_attach_id );
		...
		case "del" :
			...
			$controller = new IcmsPersistableController ( $faq_attachment_handler );
		...
		case "remove" :
			...
			$xoopsLogger->disableLogger ();
			$attachmentObj = $faq_attachment_handler->get ( $clean_attach_id );
			if ($faq_attachment_handler->delete ( $attachmentObj, true )) {
				echo 1;
			} else {
				echo 0;
			}
		...
		case 'select_plugin' :
			...
			$attachment_pluginsArray = $faq_attachment_handler->getModuleList ();
			$control->addOptionArray ( $attachment_pluginsArray );
			$control->setExtra ( 'onchange="add_module_list(this.value,0,\'\')"' );
			echo '<h2 style="margin-top:2px;">' . _CO_FAQ_ATTACHMENT_SEL_MODULE;
		...
		case 'list_module_items' :
			...
			if (empty ( $modname ) || ! file_exists ( FAQ_ROOT_PATH . 'plugins/' . $modname . '.php' )) {
				echo '<h2 style="text-align:center;">Can\'t find the Plugin</h2>';
				exit ();
			}
			
			include_once FAQ_ROOT_PATH . 'plugins/' . $modname . '.php';
			
			$showItemsTable = 'faq_'.$modname . '_showItemsTable';
			...
			$attachment_statusArray = $faq_attachment_handler->getModuleList ();
			
			echo '<h2 style="text-align:center;">' . sprintf ( _CO_FAQ_ATTACHMENT_AVALIABLE_ITEMS, $attachment_statusArray [$modname] ) . '</h2>';
			echo '<p>' . _CO_FAQ_ATTACHMENT_AVALIABLE_ITEMS_DSC . '</p>';
			
			echo $showItemsTable ( $start, $query );
			
			echo '<script>
					jQuery(document).ready(function() {
						jQuery( "#faq_addattachments_container" ).dialog( "option", "buttons", { "' . _AM_FAQ_ATTACH . '": function() { attach_items("' . $modname . '"); }, "' . _AM_FAQ_CANCEL . '": function() { jQuery(this).dialog("close"); } } );
						
						jQuery(\'#faq_addattachments_module_list a\').bind(\'click\',function () {
							jQuery(this).attr("target","_blank");
						});
					    jQuery(\'#pagenav a\').bind(\'click\',function () {
							getPagenavClick(jQuery(this),"' . $modname . '",jQuery(\'#faq_addattachments_module_list #query\').val());
							return false;
						});
						
						jQuery(\'#faq_addattachments_module_list #query\').focus();
						jQuery(\'#faq_addattachments_module_list #query_btn\').click(function(){
							add_module_list("' . $modname . '","' . $start . '",jQuery(\'#faq_addattachments_module_list #query\').val());
						});
					});
				  </script>';
