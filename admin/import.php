...
switch ( $op) {
	...
	case "importExecute" :
		...
		if (! file_exists ( $importfile_path )) {
			$errs [] = sprintf ( _AM_FAQ_IMPORT_FILE_NOT_FOUND, $importfile_path );
			$error = true;
		} else {
...
*/
		if (isset ( $importfile_select_array ) && count ( $importfile_select_array ) > 0) {
			...
			$sform = new XoopsThemeForm ( _AM_FAQ_IMPORT_SELECTION, "op", xoops_getenv ( 'PHP_SELF' ) );
			...
			$importfile_tray = new XoopsFormElementTray ( _AM_FAQ_IMPORT_SELECT_FILE, '&nbsp;' );
			$importfile_tray->addElement ( $importfile_select );
			$importfile_tray->setDescription ( _AM_FAQ_IMPORT_SELECT_FILE_DSC );
			...
			$butt_import = new XoopsFormButton ( '', '', _AM_FAQ_IMPORT, 'submit' );
			$butt_import->setExtra ( 'onclick="this.form.elements.op.value=\'importExecute\'"' );
			$button_tray->addElement ( $butt_import );
			
			$butt_cancel = new XoopsFormButton ( '', '', _AM_FAQ_CANCEL, 'button' );
			...
			*/
			...
			echo "<span style=\"color: #567; margin: 3px 0 12px 0; font-weight: bold; font-size: small; display: block; \">" . _AM_FAQ_IMPORT_NO_MODULE . "</span>";
		}
		icms_cp_footer ();
	break;
}
