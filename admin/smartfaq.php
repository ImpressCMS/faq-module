...
* Import Script
* 
* Import data from SmartFAQ 1.0.x to FAQ
* 
...
if ($op == 'start') {
	...
	if ($totalCat == 0) {
		...
		if ($totalArticles == 0) {
			$errs [] = sprintf ( _AM_FAQ_IMPORT_MODULE_FOUND_NO_ITEMS, $importFromModuleName, $totalArticles );
			$error = true;
		} else {
			echo "<span style=\"color: #567; margin: 3px 0 12px 0; font-size: small; display: block; \">" . sprintf ( _AM_FAQ_IMPORT_MODULE_FOUND, $importFromModuleName, $totalArticles, $totalCat ) . "</span>";
			
			$form = new XoopsThemeForm ( _AM_FAQ_IMPORT_SETTINGS, 'import_form', FAQ_ADMIN_URL . "$scriptname" );
			...
			$cat_label = new XoopsFormLabel ( _AM_FAQ_IMPORT_CATEGORIES, implode ( "<br />", $cat_cbox_options ) );
			$cat_label->setDescription ( _AM_FAQ_IMPORT_CATEGORIES_DSC );
			$form->addElement ( $cat_label );
			
			// SmartFAQ parent category
			$faq_category_handler = icms_getModuleHandler ( 'category' );
			$parent_cat_sel = new XoopsFormSelect(_AM_FAQ_IMPORT_PARENT_CATEGORY,'parent_category');
			$parent_cat_sel->setDescription ( _AM_FAQ_IMPORT_PARENT_CATEGORY_DSC );
			$parent_cat_sel->addOptionArray($faq_category_handler->getCategoryList());
			$form->addElement ( $parent_cat_sel );
				
			$form->addElement ( new XoopsFormHidden ( 'op', 'go' ) );
			$form->addElement ( new XoopsFormButton ( '', 'import', _AM_FAQ_IMPORT, 'submit' ) );
...
if ($op == 'go') {
	...
	while ( $arrCat = $xoopsDB->fetchArray ( $resultCat ) ) {
		// insert category into FAQ
		$categoryObj = $faq_category_handler->create ();
		...
		if (! $faq_category_handler->insert ( $categoryObj, true )) {
			$errs [] = sprintf ( _AM_FAQ_IMPORT_CATEGORY_ERROR, $arrCat ['name'] );
			$error = true;
			continue;
		}
		...
		foreach ( $groups as $group ) {
			if (! $gperm_handler->addRight ( 'category_read', $newCat ['newid'], $group, $faqModule->mid () )) {
				$errs [] = sprintf ( _AM_FAQ_IMPORT_CATEGORY_PERMISSION_ERROR, $arrCat ['name'] );
				$error = true;
				continue;
			}
		}
		if (! $gperm_handler->addRight ( 'category_write', $newCat ['newid'], 1, $faqModule->mid () )) {
			$errs [] = sprintf ( _AM_FAQ_IMPORT_CATEGORY_PERMISSION_ERROR, $arrCat ['name'] );
			$error = true;
			continue;
		}
		...
		while ( $arrArticle = $xoopsDB->fetchArray ( $resultArticles ) ) {
			// insert article
			$itemObj = & $faq_faq_handler->create ();
			
			$sts = $arrArticle ['status'];
			if ($sts < 5 || $sts == 6){
				$sts = FAQ_FAQ_STATUS_PENDING;
			}elseif ($sts == 5){
				$sts = FAQ_FAQ_STATUS_PUBLISHED;
			}elseif ($sts == 7){
				$sts = FAQ_FAQ_STATUS_OFFLINE;
			}elseif ($sts > 7){
				$sts = FAQ_FAQ_STATUS_REJECTED;
			}
			...
			if (! $faq_faq_handler->insert ($itemObj, true)) {
				$errs [] = sprintf ( "  " . _AM_FAQ_IMPORT_ARTICLE_ERROR, $arrArticle ['question'] );
				$error = true;
				continue;
			}
	...
	foreach ( $newCatArray as $oldid => $newCat ) {
		...
		$faq_category_handler->updateAll ( 'cat_pid', $newpid, $criteria );
		unset ( $criteria );
	}
	...
	$moduleObj = $module_handler->getByDirname ( 'faq' );
	$faq_module_id = $moduleObj->getVar ( 'mid' );
	...
	foreach ( $comments as $comment ) {
		...
		$comment->setVar ( 'com_modid', $faq_module_id );
		$comment->setNew ();
		if (! $comment_handler->insert ( $comment )) {
			$errs [] = sprintf ( _AM_FAQ_IMPORTED_COMMENT_ERROR, $comment->getVar ( 'com_title' ) );
			$error = true;
		}	
	}
	
	if ($error == true) {
		$endMsg = _AM_FAQ_IMPORT_ERROR . '<br />' . _ERROR . '<br />' . implode ( '<br />', $errs );
	} else {
		$endMsg = _AM_FAQ_IMPORT_SUCCESS;
	}
