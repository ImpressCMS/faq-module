...
jQuery(document).ready(function() {
	jQuery('#attach_btn').click(function(){
		jQuery.post("attachment.php",{ op: "select_plugin"},
			function(data){
				jQuery('body').append('<div id="faq_addattachments_container" title="'+_AM_FAQ_FAQ_ATTACH+'">'+data+'</div>');
				jQuery("#faq_addattachments_container").dialog({ 
					width: 800,
					height: 700,
					draggable: true,
					buttons: { Cancel: function() { jQuery(this).dialog("close"); } },
					beforeclose: function(event, ui) { jQuery('#faq_addattachments_container').remove(); }	 
				});
			}
		);
	});
	...
	jQuery('#cancel_button').click(function(){
		...
		if (faq_id == 0 && new_attachments.length > 1){
			...
			jQuery('body').append('<div id="dialog_msg" title="'+_AM_FAQ_ATTACH_WARNING+'">'+_AM_FAQ_ATTACH_WARNING_HAS_ATTACH+'</div>');
		...
		function(data, textStatus, XMLHttpRequest){
			...
			jQuery.each(data['attach_ids'],function(i, val) {
				var html  = '<li id="attachment_' +i+ '">';
					...
					html += ' <img class="show_hide_attachment" id="sh_' +data[val]['attach_id']+ '" src="'+ICMS_URL+'/modules/faq/images/'+(data[val]['attach_status'] == 1?'hide':'show')+'.gif" width="12" alt="'+(data[val]['attach_status'] == 1?_AM_FAQ_ATTACHMENT_HIDE:_AM_FAQ_ATTACHMENT_SHOW)+'" title="'+(data[val]['attach_status'] == 1?_AM_FAQ_ATTACHMENT_HIDE:_AM_FAQ_ATTACHMENT_SHOW)+'" />';
					html += ' <img class="delete_attachment" id="' +data[val]['attach_id']+ '" src="'+ICMS_IMAGES_SET_URL+'/actions/button_cancel.png" width="10" alt="'+_AM_FAQ_ATTACHMENT_DELETE+'" title="'+_AM_FAQ_ATTACHMENT_DELETE+'" />';
					html += '</li>';
...
function add_module_list(value,start,query){
	jQuery.post("attachment.php",{ op: "list_module_items", module: value, start: start, query: query},
		function(data){
			jQuery('#faq_addattachments_module_list').remove();
			jQuery("#faq_addattachments_container").append('<div id="faq_addattachments_module_list">'+data+'</div>');
		}
	);
}
...
function attach_items(module){
	...
	if (jQuery('#faq_addattachments_module_list form input:checkbox:checked').length == 0){
		jQuery('body').append('<div id="dialog_msg" title="'+_AM_FAQ_ATTACH_WARNING+'">'+_AM_FAQ_ATTACH_WARNING_MSG+'</div>');
		jQuery("#dialog_msg").dialog({ 
			modal: true,
	...
	jQuery('#faq_addattachments_module_list form input:checkbox:checked').each(function (){
		...
		jQuery.post("attachment.php",{ attach_fid: attach_fid, op: "addattachments", attach_module: module, attach_itemid: attach_itemid },
			function(data, textStatus, XMLHttpRequest){
				...
				var html  = '<li id="attachment_' +data[attach_itemid]['attach_id']+ '">';
					...
					html += ' <img class="sort_attachment" src="'+ICMS_URL+'/modules/faq/images/arrow.png" width="10" alt="'+_AM_FAQ_ATTACHMENT_SORT+'" title="'+_AM_FAQ_ATTACHMENT_SORT+'" />';
					html += ' <img class="show_hide_attachment" id="sh_' +data[attach_itemid]['attach_id']+ '" src="'+ICMS_URL+'/modules/faq/images/'+(data[attach_itemid]['attach_status'] == 1?'hide':'show')+'.gif" width="12" alt="'+(data[attach_itemid]['attach_status'] == 1?_AM_FAQ_ATTACHMENT_HIDE:_AM_FAQ_ATTACHMENT_SHOW)+'" title="'+(data[attach_itemid]['attach_status'] == 1?_AM_FAQ_ATTACHMENT_HIDE:_AM_FAQ_ATTACHMENT_SHOW)+'" />';
					html += ' <img class="delete_attachment" id="' +data[attach_itemid]['attach_id']+ '" src="'+ICMS_IMAGES_SET_URL+'/actions/button_cancel.png" width="10" alt="'+_AM_FAQ_ATTACHMENT_DELETE+'" title="'+_AM_FAQ_ATTACHMENT_DELETE+'" />';
					html += '</li>';
				jQuery('#uplist').append(html);
				jQuery(".sort_attachment").css('cursor','move');
				jQuery(".show_hide_attachment").css('cursor','pointer');
	...
	jQuery("#faq_addattachments_container").dialog("close");
}
