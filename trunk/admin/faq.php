...
function editfaq($faq_id = 0) {
	global $faq_faq_handler, $icmsModule, $icmsAdminTpl, $faqConfig, $xoopsUser;
	
	$faqObj = $faq_faq_handler->get ( $faq_id );
	...
	if (! $faqObj->isNew ()) {
		$icmsModule->displayAdminMenu ( 1, _AM_FAQ_FAQS . " > " . _CO_ICMS_EDITING );
		$sform = $faqObj->getForm ( _AM_FAQ_FAQ_EDIT, 'addfaq' );
		$sform->assign ( $icmsAdminTpl );
	
	} else {
		$icmsModule->displayAdminMenu ( 1, _AM_FAQ_FAQS . " > " . _CO_ICMS_CREATINGNEW );
		$faqObj->setVar('faq_published_date',time());
		$faqObj->setVar ( 'faq_uid', $xoopsUser->uid () );
		$sform = $faqObj->getForm ( _AM_FAQ_FAQ_CREATE, 'addfaq' );
		$sform->assign ( $icmsAdminTpl );
...
if (in_array ( $clean_op, $valid_op, true )) {
	switch ( $clean_op) {
		...
		case "addfaq" :
			...
			$controller->storeFromDefaultForm ( _AM_FAQ_FAQ_CREATED, _AM_FAQ_FAQ_MODIFIED, faq_get_page_before_form() );
		...
		case "changedField" :
		    foreach ($_POST['FaqFaq_objects'] as $k=>$v){
		       $changed = false;
		       $obj = $faq_faq_handler->get($v);
		       ...
		       if ($changed){
		           $faq_faq_handler->insert($obj);
		       }
		    }

		    redirect_header('faq.php',2,_AM_FAQ_FAQ_MODIFIED);
		
		break;
		
		case "changedField_attach" :
			foreach ( $_POST ['FaqAttachment_objects'] as $k => $v ) {
				$changed = false;
				$obj = $faq_attachment_handler->get ( $v );
				...
				if ($changed) {
					$faq_attachment_handler->insert ( $obj );
				}
			}
			if ($clean_faq_id) {
				$redir = '?faq_id=' . $clean_faq_id;
			}
			redirect_header ( 'faq.php'.$redir, 2, _AM_FAQ_ATTACHMENT_MODIFIED );
			break;
		...
		case "view" :
			...
			$xoopsModule->displayAdminMenu ( 1, _AM_FAQ_FAQS . " > " . _PREVIEW .' > '. $faqObj->getVar('faq_question','nh') );
			...
			$objectTable = new IcmsPersistableTable ( $faq_attachment_handler, $criteria, array('delete') );
			
			$objectTable->addHeader ( '<h2>'._MI_FAQ_ATTACHMENTS.'</h2>' );
			...
			$objectTable->addIntroButton ( 'addattachment', 'faq.php?op=mod&faq_id='.$clean_faq_id.'#auto_attach', _AM_FAQ_FAQ_ATTACH );
			$objectTable->addActionButton ( 'changedField_attach', false, _SUBMIT );
			
			$icmsAdminTpl->assign ( 'faq_attachment_table', $objectTable->fetch () );
			$icmsAdminTpl->display ( 'db:faq_admin_attachment.html' );
		...
		default :
			...
			$icmsModule->displayAdminMenu ( 1, _AM_FAQ_FAQS );
				
			include_once ICMS_ROOT_PATH . "/kernel/icmspersistabletable.php";
			$objectTable = new IcmsPersistableTable ( $faq_faq_handler );
			...
			$objectTable->addIntroButton ( 'addfaq', 'faq.php?op=mod', _AM_FAQ_FAQ_CREATE );
			...
			$icmsAdminTpl->assign ( 'faq_faq_table', $objectTable->fetch () );
			$icmsAdminTpl->display ( 'db:faq_admin_faq.html' );
		break;
	}
	icms_cp_footer ();
